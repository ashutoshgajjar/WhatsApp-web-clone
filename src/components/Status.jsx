  ...
  return (
    <>
      <section className='sm:w-80 md:w-88 lg:w-100 flex-1 py-2 bg-white dark:bg-zinc-900 dark:text-white border-x border-black/10 dark:border-white/10'>
        ...
        <div className='space-y-2 px-3 h-[calc(100vh-10.85rem)] overflow-y-auto'>
          {mockStatus.map((user, idx) => (
            <div
              key={idx}
              className='grid grid-cols-[40px_1fr] items-center relative gap-2 p-2 rounded-lg hover:bg-stone-100 dark:hover:bg-zinc-700 cursor-pointer group'
            >
              <img
                src={user.avatar}
                alt={`${user.name} Avatar`}
                className='size-9 outline-2 outline-offset-2 outline-green-500 rounded-full'
              />
